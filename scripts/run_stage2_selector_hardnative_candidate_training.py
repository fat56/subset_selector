#!/usr/bin/env python3
"""Train a Stage 2 selector against 0003 hard-native candidate labels."""
...
from vggt_omega_selector.selectors.models import FixedKSetSelector  # noqa: E402
...
class CandidateSetScorer(nn.Module):
    """Score fixed-K candidate subsets from contextual per-frame features."""
    ...
    def forward(
        ...
        if candidate_masks.ndim != 3:
            raise ValueError(f"Expected candidate_masks [B,M,N], got {tuple(candidate_masks.shape)}")
        ...
        pos = torch.linspace(0.0, 1.0, frame_count, device=hidden.device, dtype=hidden.dtype)
        ...
        selected_fraction = selected.sum(dim=-1, keepdim=True) / frame_mask.sum(dim=-1, keepdim=True).clamp_min(1).to(hidden.dtype)[:, None, :]
        stats = torch.cat([selected_fraction, pos_mean, pos_std, pos_min, pos_max, span], dim=-1)

        candidate_features = torch.cat([mean, std, stats], dim=-1)
...
class HardNativeCandidateDataset(Dataset[dict[str, Any]]):
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Train 0004 main_v3 hard-native candidate selector.")
    parser.add_argument(
        "--labels-csv",
    ...
    parser.add_argument("--model-kind", choices=["candidate_set", "frame_score"], default="candidate_set")
...
def load_examples(config: HardNativeTrainConfig) -> list[SceneExample]:
    ...
    for scene_id, rows in rows_by_scene.items():
        ...
        if full_job is None:
            continue
        full_images = read_image_list(resolve(full_job["image_list"]))
        ...
        if len(candidates) < 3:
            continue
        full_token_path = resolve(rows[0]["full_token_path"])
        if not full_token_path.is_file():
            continue
        raw_examples.append(
            SceneExample(
                scene_id=scene_id,
                ...
                feature_path=config.feature_cache_dir / f"{scene_id}.pt",
    ...
    ensure_feature_cache(examples, config)
    return examples
...
def assign_splits(examples: list[SceneExample], config: HardNativeTrainConfig) -> list[SceneExample]:
        ...
        for index, example in enumerate(dataset_examples):
            ...
            assigned.append(
                SceneExample(
                    ...
                    dataset=example.dataset,
                    split=split,
                    full_token_path=example.full_token_path,
...
def ensure_feature_cache(examples: list[SceneExample], config: HardNativeTrainConfig) -> None:
    missing = [example for example in examples if config.rebuild_feature_cache or not example.feature_path.is_file()]
    if not missing:
        print(json.dumps({"event": "feature_cache_skip", "scenes": len(examples)}), flush=True)
        return
    started = time.time()
    total = len(missing)
    for index, example in enumerate(missing, start=1):
        frame_features = load_frame_features(example.full_token_path)
        if frame_features.shape[0] != example.full_image_count:
            raise ValueError(
                f"Feature count mismatch for {example.scene_id}: "
                f"features={frame_features.shape[0]} full_images={example.full_image_count}"
            )
        example.feature_path.parent.mkdir(parents=True, exist_ok=True)
        torch.save(
            {
                "scene_id": example.scene_id,
                "source": str(example.full_token_path),
                "frame_features": frame_features.to(dtype=torch.float16),
            },
...
def load_frame_features(token_path: Path) -> torch.Tensor:
    tokens = torch.load(token_path, map_location="cpu")
    if isinstance(tokens, dict):
        tokens = tokens.get("camera_and_register_tokens", tokens.get("tokens"))
    if not isinstance(tokens, torch.Tensor):
        raise TypeError(f"Unsupported token payload in {token_path}")
    if tokens.ndim != 4 or tokens.shape[0] != 1 or tokens.shape[2] < 2:
        raise ValueError(f"Expected tokens [1,N,R+1,C], got {tuple(tokens.shape)} from {token_path}")
    per_frame = tokens.squeeze(0).float()
    camera = per_frame[:, 0, :]
    registers = per_frame[:, 1:, :]
    register_mean = registers.mean(dim=1)
    register_max = registers.amax(dim=1)
    register_std = registers.std(dim=1, unbiased=False)
    frame_pos = torch.linspace(0.0, 1.0, per_frame.shape[0], dtype=torch.float32).unsqueeze(1)
    return torch.cat([camera, register_mean, register_max, register_std, frame_pos], dim=-1)


def write_dataset_summary(examples: list[SceneExample], config: HardNativeTrainConfig) -> None:
        ...
        rows.append(
            {
                ...
                "full_token_path": str(example.full_token_path),
            }
        )
...
def train_selector(config: HardNativeTrainConfig, examples: list[SceneExample]) -> dict[str, Any]:
    train_ds = HardNativeCandidateDataset(examples, "train")
    val_ds = HardNativeCandidateDataset(examples, "val")
    test_ds = HardNativeCandidateDataset(examples, "test")
    ...
    model = build_model(config, input_dim, max_frames)
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for batch in train_loader:
            ...
            loss, stats = hardnative_loss(scores, target_errors, candidate_valid, config)
...
def build_model(config: HardNativeTrainConfig, input_dim: int, max_frames: int) -> nn.Module:
    if config.model_kind == "frame_score":
        return FixedKSetSelector(
            input_dim=input_dim,
    ...
    if config.model_kind == "candidate_set":
        return CandidateSetScorer(
            input_dim=input_dim,
            hidden_dim=config.hidden_dim,
            num_layers=config.num_layers,
            num_heads=config.num_heads,
...
def hardnative_loss(
    ...
    loss = config.rank_weight * rank_loss + config.ce_weight * ce_loss
    return loss, {
