...
目标：先建强 baseline 和 sparse-view GS 评测流程，不训练 selector。

必须产出：

- random/uniform/k-center/register-k-center 等基础方法的 selected indices。
- 每个子集的 register/readout similarity。
- FastGS/3DGS 类后端评测指标：PSNR、SSIM、LPIPS、pose/depth 可选指标。
...
```text
register cosine similarity 与 20% FastGS PSNR/SSIM/LPIPS 是否显著相关？
```

建议通过标准：

- Spearman rho >= 0.5。
- 20% ratio 下多种 baseline、多个 scene 上方向稳定。
- 不只看均值，也检查失败场景。
